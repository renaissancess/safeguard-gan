...
state_dict = torch.load('result/netG_025.pth')
netG.load_state_dict(state_dict)

#图像读入与预处理
transforms = torchvision.transforms.Compose([
    torchvision.transforms.Resize(opt.imageSize),# 裁剪成96*96
...
for epoch in range(1, opt.epoch + 1):
    for i, (real_img,_) in enumerate(dataloader):# [64,3,96,96]
        ...
        print('[%d/%d][%d/%d] Loss_D: %.3f Loss_G %.3f'
              % (epoch, opt.epoch+25, i, len(dataloader), d_loss.item(), g_loss.item()))

    save_image(fake_img.data[0],
                      '%s/fake_samples_epoch_%03d.png' % (opt.result_path, epoch+25),
                      normalize=True)

torch.save(netG.state_dict(), '%s/netG_%03d.pth' % (opt.result_path, epoch+25))


#加载模型

# model = Generator(100,64) # init your model class, build the graph shape

# state_dict = torch.load('result/netG_025.pth')
# model.load_state_dict(state_dict)
# model.eval()

# noise = torch.randn(1,100, 1, 1) # 生成100维的随机噪声 [64,100,1,1]
# fake_img = model(noise)

# save_image(fake_img.data, 'fake_samples.png' , normalize=True)